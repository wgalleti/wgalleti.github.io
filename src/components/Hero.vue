...
const capabilityKeys = ['heroCapability1', 'heroCapability2', 'heroCapability3', 'heroCapability4']
...
<template>
  <section class="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden">
    ...
    <div class="container-section relative z-10">
      <div class="grid gap-14 lg:grid-cols-[minmax(0,1fr)_320px] lg:items-end">
        <div class="max-w-4xl">
        ...
        <div :class="['flex flex-wrap gap-3 mb-10 transition-all duration-700 delay-[375ms]', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
          <span
            v-for="key in capabilityKeys"
            :key="key"
            class="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-4 py-2 text-xs font-display font-medium text-slate-300"
          >
        ...
        <div :class="['flex flex-col sm:flex-row gap-4 mb-20 transition-all duration-700 delay-[450ms]', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
          ...
          <button @click="document.getElementById('process')?.scrollIntoView({ behavior: 'smooth' })" class="btn-secondary text-base !py-4 !px-8">
        ...
        <aside :class="['card-glass hidden lg:block p-7 transition-all duration-700 delay-[500ms]', visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8']">
          <div class="mb-6">
            <p class="text-xs font-display font-semibold uppercase tracking-[0.24em] text-slate-500 mb-3">
              {{ t('heroPanelEyebrow') }}
            </p>
            <h2 class="text-2xl font-display font-bold text-white leading-tight">
              {{ t('heroPanelTitle') }}
            </h2>
          </div>
          ...
          <div class="space-y-4">
            <div class="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
              <p class="text-xs font-display font-semibold uppercase tracking-[0.18em] text-brand-300 mb-2">{{ t('heroPanelBlock1Label') }}</p>
              <p class="text-sm text-slate-300 font-body leading-relaxed">{{ t('heroPanelBlock1Text') }}</p>
            </div>
            <div class="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
              <p class="text-xs font-display font-semibold uppercase tracking-[0.18em] text-accent-300 mb-2">{{ t('heroPanelBlock2Label') }}</p>
              <p class="text-sm text-slate-300 font-body leading-relaxed">{{ t('heroPanelBlock2Text') }}</p>
            </div>
            <div class="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
              <p class="text-xs font-display font-semibold uppercase tracking-[0.18em] text-emerald-300 mb-2">{{ t('heroPanelBlock3Label') }}</p>
              <p class="text-sm text-slate-300 font-body leading-relaxed">{{ t('heroPanelBlock3Text') }}</p>
            </div>
          </div>
        </aside>
      </div>
